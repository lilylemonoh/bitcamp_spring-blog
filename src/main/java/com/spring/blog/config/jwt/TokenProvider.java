...
public class TokenProvider {
    ...
    private final JwtProperties jwtProperties; // getter를 이용해 properties파일에 작성한 데이터 가져오기
    ...
    // 검증로직은 유효한지, 혹은 유효하지 않은지만 리턴하면 되므로 boolean 을 리턴합니다.
    // 발급된 토큰을 입력받았을 때, 유효화면 true, 아니면 false를 리턴합니다.
    public boolean validToken(String token) {
        try {
            Jwts.parser() // 토큰을 먼저 파싱(읽어들임)하고
                    .setSigningKey(jwtProperties.getSecretKey()) // 복호화 시에는 암호화 때 사용한 비밀키를 입력
    ...
    // 클레임 정보(실제 개인정보)는 외부에서 직접적으로 호출할 수 없고, 오로지 토큰 제공 클래스 내부에서만 호출되도록 처리
