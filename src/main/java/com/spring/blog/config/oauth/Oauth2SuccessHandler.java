...
// 소셜로그인 성공 시 처리할 내역들에 대해 기술하는 클래스
...
public class Oauth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private static final String REDIRECT_PATH = "/blog/list";


    private final TokenProvider tokenProvider;
    private final RefreshTokenRepository refreshTokenRepository;
    // @Configuration으로 등록된 설정 클래스 내부 메서드에 @Bean 어노테이션이 붙어있다면
    // 해당 메서드가 리턴하는 빈도 빈 컨테이너에 자동으로 등록됩니다.
    // 아래 클래스 인스턴스의 경우는 BasicSecurityConfig 클래스 내부 메서드에 의해 생성된 빈을 주입받습니다.
    private final OAuth2AuthorizationRequestBasedOnCookieRepository authorizationRequestRepository;
    private final UsersService usersService; // 임포트해야 자동 주입
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        // 리프레시 토큰도 JWT 토큰의 일종이므로 TokenProvider로 생성
        String refreshToken = tokenProvider.generateToken(user, REFRESH_TOKEN_DURATION);
        // 함수를 호출해서 뭔가 로직을 처리하는 부분은 대부분 리팩토링을 위해 외부 함수에 실행로직을 빼둔 경우입니다.
    ...
    private void saveRefreshToken(Long userId, String newRefreshToken) {
        ...
        // 위에서 얻어온 토큰정보가 있다면 새 토큰으로 갱신, 없다면 신규 발급
